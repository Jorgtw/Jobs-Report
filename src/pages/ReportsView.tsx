              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3">
                ...
                <div className="md:col-span-2 space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  ...
                  <div className="hidden sm:grid grid-cols-12 gap-2 px-2 mb-1 pr-10 sm:pr-0">
                    ...
                    <div className="col-span-1 px-1 text-[10px] font-extrabold text-amber-500 uppercase text-center sm:border-l sm:border-transparent sm:pl-1">{t('reports.headerExtra')}</div>
                    <div className="col-span-1 px-1 text-[10px] font-extrabold text-red-500 uppercase text-center sm:border-l sm:border-transparent sm:pl-1">{t('reports.headerFestive')}</div>
                    <div className="col-span-1 px-1 text-[10px] font-extrabold text-indigo-500 uppercase text-center sm:border-l sm:border-transparent sm:pl-1">{t('reports.headerNight')}</div>
                    <div className="col-span-2 px-1 text-[10px] font-extrabold text-slate-400 uppercase text-center sm:border-l sm:border-transparent sm:pl-1 pr-8">{t('reports.headerTotal')}</div>
                  </div>
                  {user.role !== 'operator' && formData.additionalWorkers.map((aw, idx) => (
                    <div key={idx} className="bg-white p-2 rounded-xl border border-slate-200 grid grid-cols-12 gap-2 items-center shadow-sm relative pr-10 sm:pr-0">
                      ...
                      <div className="col-span-3 sm:col-span-1 flex flex-col gap-0.5 sm:border-l sm:border-slate-200 sm:pl-1"><label className="text-[9px] font-extrabold text-amber-500 uppercase ml-1 tracking-tight sm:hidden">{t('reports.headerExtra')}</label><input type="number" step="0.25" value={aw.overtimeHours === 0 ? 0 : aw.overtimeHours || ''} onChange={e => updateWorker(idx, { overtimeHours: parseFloat(e.target.value) || 0 })} placeholder="0" className={`${inputClasses} w-full text-center text-amber-600 font-bold bg-amber-50 border-amber-200 px-0 text-[11px] sm:text-sm`} /></div>
                      <div className="col-span-3 sm:col-span-1 flex flex-col gap-0.5 sm:border-l sm:border-slate-200 sm:pl-1"><label className="text-[9px] font-extrabold text-red-500 uppercase ml-1 tracking-tight sm:hidden">{t('reports.headerFestive')}</label><input type="number" step="0.25" value={aw.festiveHours === 0 ? 0 : aw.festiveHours || ''} onChange={e => updateWorker(idx, { festiveHours: parseFloat(e.target.value) || 0 })} placeholder="0" className={`${inputClasses} w-full text-center text-red-600 font-bold bg-red-50 border-red-200 px-0 text-[11px] sm:text-sm`} /></div>
                      <div className="col-span-3 sm:col-span-1 flex flex-col gap-0.5 sm:border-l sm:border-slate-200 sm:pl-1"><label className="text-[9px] font-extrabold text-indigo-500 uppercase ml-1 tracking-tight sm:hidden">{t('reports.headerNight')}</label><input type="number" step="0.25" value={aw.nightHours === 0 ? 0 : aw.nightHours || ''} onChange={e => updateWorker(idx, { nightHours: parseFloat(e.target.value) || 0 })} placeholder="0" className={`${inputClasses} w-full text-center text-indigo-600 font-bold bg-indigo-50 border-indigo-200 px-0 text-[11px] sm:text-sm`} /></div>
                      <div className="col-span-12 sm:col-span-2 flex flex-col gap-0.5 sm:border-l sm:border-slate-200 sm:pl-1"><label className="text-[9px] font-extrabold text-slate-400 uppercase ml-1 tracking-tight sm:hidden">{t('reports.headerTotal')}</label><input type="number" step="0.01" value={aw.manualTotalHours !== undefined ? aw.manualTotalHours : ''} onChange={e => updateWorker(idx, { manualTotalHours: e.target.value === "" ? undefined : parseFloat(e.target.value) })} placeholder={db.calculateTotalHours(aw.startTime, aw.endTime, aw.breakHours).toFixed(2)} className="w-full px-1 py-1 bg-white border border-slate-200 rounded-lg text-center font-black text-blue-600 outline-none h-[30px] text-[11px] sm:text-sm" /></div>
                  ...
                  <div className="bg-white p-2 rounded-xl border border-blue-200 grid grid-cols-12 gap-2 items-center shadow-sm relative pr-10 sm:pr-0">
                    ...
                    <div className="col-span-3 sm:col-span-1 flex flex-col gap-0.5"><label className="text-[9px] font-extrabold text-slate-400 uppercase ml-1 tracking-tight sm:hidden">{t('reports.headerBreak')}</label><input type="number" step="0.25" value={formData.breakHours === 0 ? 0 : formData.breakHours || ''} onChange={e => setFormData({ ...formData, breakHours: parseFloat(e.target.value) || 0 })} className={`${inputClasses} w-full text-center px-0.5 text-[11px] sm:text-sm`} /></div>
                    <div className="col-span-3 sm:col-span-1 flex flex-col gap-0.5 sm:border-l sm:border-slate-200 sm:pl-1"><label className="text-[9px] font-extrabold text-amber-500 uppercase ml-1 tracking-tight sm:hidden">{t('reports.headerExtra')}</label><input type="number" step="0.25" value={formData.overtimeHours === 0 ? 0 : formData.overtimeHours || ''} onChange={e => setFormData({ ...formData, overtimeHours: parseFloat(e.target.value) || 0 })} placeholder="0" className={`${inputClasses} w-full text-center text-amber-600 font-bold bg-amber-50 border-amber-200 px-0 text-[11px] sm:text-sm`} /></div>
                    <div className="col-span-3 sm:col-span-1 flex flex-col gap-0.5 sm:border-l sm:border-slate-200 sm:pl-1"><label className="text-[9px] font-extrabold text-red-500 uppercase ml-1 tracking-tight sm:hidden">{t('reports.headerFestive')}</label><input type="number" step="0.25" value={formData.festiveHours === 0 ? 0 : formData.festiveHours || ''} onChange={e => setFormData({ ...formData, festiveHours: parseFloat(e.target.value) || 0 })} placeholder="0" className={`${inputClasses} w-full text-center text-red-600 font-bold bg-red-50 border-red-200 px-0 text-[11px] sm:text-sm`} /></div>
                    <div className="col-span-3 sm:col-span-1 flex flex-col gap-0.5 sm:border-l sm:border-slate-200 sm:pl-1"><label className="text-[9px] font-extrabold text-indigo-500 uppercase ml-1 tracking-tight sm:hidden">{t('reports.headerNight')}</label><input type="number" step="0.25" value={formData.nightHours === 0 ? 0 : formData.nightHours || ''} onChange={e => setFormData({ ...formData, nightHours: parseFloat(e.target.value) || 0 })} placeholder="0" className={`${inputClasses} w-full text-center text-indigo-600 font-bold bg-indigo-50 border-indigo-200 px-0 text-[11px] sm:text-sm`} /></div>
                ...
                <div className="md:col-span-2 space-y-3 bg-amber-50 p-4 rounded-2xl border border-amber-100">
                  ...
                  {formData.expenses.map((exp: any, idx: number) => (
                    <div key={idx} className="bg-white p-3 rounded-xl border border-amber-200 flex flex-col gap-3 shadow-sm relative pr-10">
