...
export class PdfRenderer implements ReportRenderer {
  async render(document: ReportDocument, filename: string): Promise<void> {
    ...
    const metaLines = [
      `${lang === 'en' ? 'Company' : (lang === 'es' ? 'Empresa' : 'Azienda')}: ${document.metadata.companyName}`,
      document.metadata.clientName ? `${lang === 'en' ? 'Client' : (lang === 'es' ? 'Cliente' : 'Cliente')}: ${document.metadata.clientName}` : null,
      document.metadata.projectName ? `${lang === 'en' ? 'Project' : (lang === 'es' ? 'Proyecto' : 'Progetto')}: ${document.metadata.projectName}` : null,
      `${lang === 'en' ? 'Generated by' : (lang === 'es' ? 'Generado por' : 'Generato da')}: ${document.metadata.generatedBy}`,
      `${lang === 'en' ? 'Date' : (lang === 'es' ? 'Fecha' : 'Data generazione')}: ${this.formatDate(document.metadata.generatedAt)}`
    ...
    if (filterEntries.length > 0) {
      ...
      doc.text(lang === 'en' ? 'Filters applied:' : (lang === 'es' ? 'Filtros aplicados:' : 'Filtri applicati:'), 14, startY);
    ...
    document.sections.forEach((section, sIndex) => {
      ...
      section.blocks.forEach(block => {
        startY = this.renderBlock(doc, block, startY, lang);
  ...
  private renderBlock(doc: jsPDF, block: ReportBlock, startY: number, lang: Language): number {
    switch (block.type) {
      ...
      case ReportBlockType.DASHBOARD:
        ...
        autoTable(doc, {
          startY: startY,
          head: [[lang === 'en' ? 'Indicator' : (lang === 'es' ? 'Indicador' : 'Indicatore'), lang === 'en' ? 'Value' : (lang === 'es' ? 'Valor' : 'Valore')]],
          body: dashboardBody,
          theme: 'grid',
